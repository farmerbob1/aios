return {
    -- Window
    window_bg              = 0x002D2D2D,
    window_border          = 0x00555555,
    titlebar_bg            = 0x003C3C3C,
    titlebar_bg_inactive   = 0x00333333,
    titlebar_text          = 0x00FFFFFF,
    titlebar_height        = 28,

    -- General text
    text_primary           = 0x00FFFFFF,
    text_secondary         = 0x00AAAAAA,
    text_placeholder       = 0x00666666,
    text_disabled          = 0x00555555,

    -- Accent
    accent                 = 0x00FF8800,
    accent_hover           = 0x00FFA040,
    accent_text            = 0x00FFFFFF,

    -- Button
    button_normal          = 0x003A3A3A,
    button_hover           = 0x004A4A4A,
    button_pressed         = 0x002A2A2A,
    button_disabled        = 0x00333333,
    button_text            = 0x00FFFFFF,
    button_text_disabled   = 0x00666666,
    button_radius          = 4,
    button_height          = 32,
    button_padding_h       = 12,

    -- TextField
    field_bg               = 0x00222222,
    field_border           = 0x00555555,
    field_border_focus     = 0x00FF8800,
    field_text             = 0x00FFFFFF,
    field_cursor           = 0x00FFFFFF,
    field_selection        = 0x00FF8800,
    field_height           = 28,
    field_padding          = 6,

    -- Checkbox
    checkbox_size          = 16,
    checkbox_border        = 0x00666666,
    checkbox_checked_bg    = 0x00FF8800,
    checkbox_check_color   = 0x00FFFFFF,

    -- Slider
    slider_track           = 0x00444444,
    slider_track_height    = 4,
    slider_thumb_radius    = 8,
    slider_thumb_color     = 0x00FF8800,

    -- ScrollView / scrollbar
    scrollbar_width        = 10,
    scrollbar_track        = 0x00333333,
    scrollbar_thumb        = 0x00555555,
    scrollbar_thumb_hover  = 0x00777777,
    scroll_speed           = 48,

    -- ListView
    list_item_height       = 24,
    list_selection         = 0x00FF8800,
    list_hover             = 0x003A3A3A,
    list_stripe            = 0x00323232,

    -- TabView
    tab_height             = 32,
    tab_bg                 = 0x00333333,
    tab_active_bg          = 0x002D2D2D,
    tab_text               = 0x00AAAAAA,
    tab_text_active        = 0x00FFFFFF,
    tab_indicator          = 0x00FF8800,
    tab_indicator_height   = 2,

    -- Menu
    menu_bg                = 0x003A3A3A,
    menu_border            = 0x00555555,
    menu_hover             = 0x00FF8800,
    menu_text              = 0x00FFFFFF,
    menu_shortcut_text     = 0x00888888,
    menu_separator         = 0x00444444,

    -- Dialog
    dialog_overlay_alpha   = 128,
    dialog_bg              = 0x003A3A3A,

    -- FileItem
    file_selection         = 0x00FF8800,
    file_hover             = 0x003A3A3A,
    file_grid_cell         = 72,
    file_text_shadow       = 0x00000000,

    -- AppIcon
    app_cell_size          = 80,
    app_taskbar_size       = 24,
    app_active_indicator   = 0x00FF8800,
    badge_bg               = 0x000000FF,
    badge_text             = 0x00FFFFFF,

    -- ProgressBar
    progress_bg            = 0x00333333,
    progress_fill          = 0x00FF8800,
    progress_height        = 16,

    -- Separator
    separator_color        = 0x00444444,

    -- Tooltip
    tooltip_bg             = 0x00444444,
    tooltip_border         = 0x00666666,
    tooltip_text           = 0x00FFFFFF,

    -- Window Manager
    taskbar_bg             = 0x002A2A2A,
    taskbar_height         = 32,
    desktop_bg             = 0x00283040,
    text_warning           = 0x00FFCC00,
    text_danger            = 0x00FF4444,
    cursor_color           = 0x00FFFFFF,

    -- General
    focus_outline          = 0x00FF8800,
    focus_outline_width    = 2,
    border_radius          = 4,
    spacing_sm             = 4,
    spacing_md             = 8,
    spacing_lg             = 16,
    padding_sm             = 4,
    padding_md             = 8,
    padding_lg             = 16,
}
